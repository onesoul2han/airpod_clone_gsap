...
// const a = 10;
// const b = a.toString().padStart(4, "0"); // 지정된 숫자에 대해 첫 번째 파라미터 길이, 두 번째 파라미터 문자로 채워줌
...
tl1
  .add("start0")
  .to(mainText, { opacity: 0, duration: 3, y: -500 }, "start0") // ;표 없이 끝내 효과 연결
  .to(
    airpods,
    {
      duration: 8,
      frame: 140,
      snap: "frame", // frame을 snap으로 지정하면, frame이 정수로 존재하도록 강제함
      ease: "none",
      onUpdate: () => {
        imgObj.setAttribute("src", images[airpods.frame]);
      },
    },
    "start0" // 시작 시점 지정 : 작업 생성하는 이름
  )
  .add("start1")
...
let scene1 = new ScrollMagic.Scene({
  ...
  duration: "4000",
  triggerHook: 0.1, // 0.1 = 10% from the top of the viewpoint
})
  .setTween(tl1)
  .setPin(".section-01")
  .addTo(controller) //컨트롤러에 추가
  .addIndicatiors(); //효과를 확인하기 위한 인디케이터 추가
